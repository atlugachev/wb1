...
if ($action === 'login') {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $pdo->prepare("SELECT id, password_hash FROM users2 WHERE login = ?");
        $stmt->execute([$login]);
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO users2 (fio, phone, email, birthdate, gender, bio, contract, login, password_hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $pdo->prepare("UPDATE users2 SET fio = ?, phone = ?, email = ?, birthdate = ?, gender = ?, bio = ?, contract = ? WHERE id = ?");
